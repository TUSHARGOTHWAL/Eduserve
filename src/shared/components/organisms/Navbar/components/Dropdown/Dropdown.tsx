...
interface DropdownProps extends React.HTMLAttributes<HTMLDivElement> {
  data: INavbarData[]
  type: "Courses" | "Careers" | "For Companies"
  resetState?: boolean
}
...
export const Dropdown: React.FC<DropdownProps> = ({
  ...
  useEffect(() => {
    if (resetState) handleSelectCategory(data[0])
  }, [resetState])
  ...
  function handleActiveCategory(category: INavbarData) {
    return category.category === selectedCategory.category ? "text-orange-400" : "text-dark-tertiary"
  }

  if (type === "For Companies") {
    return (
      <div
        ...
        <div className="bg-[#262626] h-[500px] rounded-sm py-6 px-3 lg:p-6">
          ...
          <ul className="flex flex-col w-full">
            {selectedCategory.items.map((item, index) => (
              <li
                key={index}
                className="py-2 cursor-pointer hover:underline text-dark-tertiary hover:text-orange-500"
              >
                {item.name}
              </li>
            ))}
    ...
    >
      <div className="bg-[#262626] h-[500px] rounded-sm py-6 px-3 lg:p-6 grid grid-cols-2">
        <div className="col-span-1 pr-3 border-r border-r-dark-secondary">
          <h3 className="text-xl pb-4 lg:pb-7">Categories</h3>
          <ul className="flex flex-col w-full">
            {data.map((item, index) => (
              <li
              ...
              >
                ...
                <CaretDown
                  size="16"
                  className="-rotate-90 group-hover:to-orange-500"
                />
              </li>
            ))}
          </ul>
        ...
        <div className="col-span-1 pl-6">
          ...
          <ul className="flex flex-col w-full">
            {selectedCategory.items.map((item, index) => (
              <li
                key={index}
                className="pb-[6px] lg:py-2 text-sm lg:text-base flex items-center gap-x-2 cursor-pointer hover:underline text-dark-tertiary hover:text-orange-500"
              >
                {item.name}{" "}
                {item.new && (
                  <div className="px-2 hidden lg:block w-fit rounded-sm bg-orange-400 text-dark-main">
                    New
                  </div>
                )}
              </li>
